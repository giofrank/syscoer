...
public function listado_usuarios(){
  ...
  $roles = User::FullRole($usuarios->id);
  // $arrayroles = json_decode( json_encode( $roles ), true );
  $usuarios->roles = $roles;
  }
  // dd($usuarios);exit();

  // $array1 = json_decode( json_encode( $usuarios ), true );
	// $usuarios=User::paginate(100);

	return view("listados.listado_usuarios")->with(compact('users', $usuarios));
}
...
public function crear_usuario(Request $request){
	...
	$reglas=[  
              'num_doc' => 'required|max:8|unique:persona',
	...
	$mensajes=[  
              'num_doc.unique' => 'El DNI ya se encuentra registrado en la base de datos', 
              'apell_pater.required' => 'El apellido es requerido',
              'apell_mat.required' => 'El apellido es requerido',
	...
	$usuario=new Persona;
	$usuario->num_doc=strtoupper( $request->input("num_doc")) ;
	$usuario->apell_pater=strtoupper( $request->input("apell_pater") ) ;
  $usuario->apell_mat=strtoupper( $request->input("apell_mat") ) ;
